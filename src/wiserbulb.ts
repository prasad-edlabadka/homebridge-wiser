...
export class WiserBulb extends WiserSwitch {
  ...
  setupService(): Service {
    ...
    if (this.accessory.context.device.wiserProjectGroup.dimmable) {
      service
  ...
  async getLevel(): Promise<CharacteristicValue> {
    ...
    this.platform.log.debug(
      `Get on state ${this.name}(${this.id}) ${this.level}`,
    );
    return percent;
  }
  ...
  async setLevel(value: CharacteristicValue) {
    ...
    this.platform.log.debug(
      `Homekit set ${this.name} to ${this.level} (${this.toWiserLevel(
        this.level,
      )})`,
    );
    this.wiser.setGroupLevel(
      this.device.wiserProjectGroup.address,
      this.toWiserLevel(this.level),
    );
  }
